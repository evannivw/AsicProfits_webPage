...
class AddPartnerPage extends StatefulWidget {
  AddPartnerPage({@required this.currentPartner, this.callback});
  PartnersModel? currentPartner;
  Function? callback;
  @override
  State<StatefulWidget> createState() {
    return _AddPartnerPage();
  }
}

class _AddPartnerPage extends PageWidget<AddPartnerPage> {
  AddPartnerPageController controller = AddPartnerPageController();
  ...
  void initState() {
    ...
    super.initState();
    controller.fillTextsControllers(widget.currentPartner ?? PartnersModel());
  }
  ...
  Widget mainView() {
    return Column(
      ...
      children: [
        inputWidget("Name", controller.nameTEC),
        inputWidget("Web page", controller.webPageTEC),
        inputWidget("Country name", controller.countryNameTEC),
        imageInput(
            "Country image",
            !controller.countryImageURL.isEmpty
                ? "Image Loaded"
                : "Load country image", () {
          ImagePicker.pick((p0) async {
            UIHelper().showLoading(context);
            var url = await FirebaseStorage()
                .uploadImage(p0, "partner/" + controller.id, "country");
            UIHelper().hideLoading(context);
            setState(() {
              controller.countryImageURL = url;
            });
          });
        }),
        inputWidget("Distributor", controller.distributorTEC),
        inputWidget("Payment", controller.paymentTEC),
        dropDownWidget("Trust", ["trust", "disable"], (str) {
        ...
        imageInput(
          ...
          ImagePicker.pick((p0) async {
            UIHelper().showLoading(context);
            var url = await FirebaseStorage()
                .uploadImage(p0, "partner/" + controller.id, "image");
            UIHelper().hideLoading(context);
            setState(() {
        ...
        inputWidget("Visit Link", controller.visitLinkTEC, maxLength: 500),
        VerticalSpacing(
          height: 40,
        ),
        Row(
          ...
          children: [
            BasicButton(
              ...
              onPressed: () {
                UIHelper().showLoading(context);
                controller.save(widget.currentPartner ?? PartnersModel(), () {
                  UIHelper().hideLoading(context);

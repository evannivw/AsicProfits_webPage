...
class AddHostingPage extends StatefulWidget {
  AddHostingPage({@required this.currentHost, this.callback});
  HostingFacilitiesModel? currentHost;
  Function? callback;
  @override
...
class _AddHostingPage extends PageWidget<AddHostingPage> {
  AddHostingPageController controller = AddHostingPageController();
  ...
  void initState() {
    ...
    super.initState();
    controller
        .fillTextsControllers(widget.currentHost ?? HostingFacilitiesModel());
  }
  ...
  Widget mainView() {
    return Column(
      ...
      children: [
        inputWidget("Name place", controller.namePlaceTEC),
        inputWidget("State name", controller.nameStateTEC),
        inputWidget("Country name", controller.nameCountryTEC),
        inputWidget("Price", controller.priceTEC),
        inputWidget("MQO", controller.MQOTEC),
        inputWidget("Security", controller.securityTEC),
        inputWidget("Energy", controller.energyTEC),
        Divider(
          color: DocColors.gray.getValue(),
          height: 60,
          thickness: 0.25,
        ),
        dropDownWidget("Availability", ["enable", "disable"], (str) {
          ...
          children: [
            BasicButton(
              ...
              onPressed: () {
                ...
                controller.save(widget.currentHost ?? HostingFacilitiesModel(),
                    () {
              ...
              onPressed: () {
                ImagePicker.pick((p0) async {
                  ...
                  var url = await FirebaseStorage()
                      .uploadImage(p0, "hostings/" + controller.id, "image");
                  UIHelper().hideLoading(context);
                  setState(() {
